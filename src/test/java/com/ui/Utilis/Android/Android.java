    ...
    public class Android extends Ios {
        ...
        protected void AndroidNative(ITestContext iTestContext, XmlTest xmlTest) {
            UiAutomator2Options androidcaps=new UiAutomator2Options();
            ...
            try {
                if (executionType.equalsIgnoreCase("remote")) {
                    if (iTestContext.getSuite().getParameter("AppType").equalsIgnoreCase("AndroidBrowser")) {
                        ...
                        androidcaps.setPlatformName(tEnv().getAndroidOSVersion().trim());
                        androidcaps.setPlatformVersion(tEnv().getAndroidDevice().trim());
                        androidcaps.setApp(xmlTest.getParameter("appPath").trim());
                    ...
                    androidcaps.setPlatformName(ANDROID);
                    androidcaps.setDeviceName("emulator-5554");
                    androidcaps.setPlatformVersion("15.0");
    //                androidcaps.setCapability(MobileCapabilityType.APP, iTestContext.getSuite().getParameter("appPath").trim());
                    androidcaps.setApp("");
                    androidcaps.setAutomationName("uiautomator2");
                    androidcaps.setCapability("autoAcceptAlerts", true);
                    androidcaps.setCapability("appPackage", "com.t2s.foodhub");
                    androidcaps.setCapability("appActivity", "com.t2s.MainActivity");
                    if (iTestContext.getSuite().getParameter("ProductName")!=null&&iTestContext.getSuite().getParameter("ProductName").equalsIgnoreCase("digitalBoard")) {
                        androidcaps.setCapability("deviceOrientation", "landscape");
                    }
                    if (xmlTest.getParameter("AppType")!=null&&xmlTest.getParameter("AppType").equalsIgnoreCase("AndroidBrowser")) {
                        androidcaps.setCapability("browserName","Chrome");

                    }
                    androidcaps.setCapability("appium:newCommandTimeout", "10000");
    ...
    //                        logger.info("  AndroidDriver DO NORMAL start");
                    ...
                    System.out.println(tEnv().getAppiumURL());
